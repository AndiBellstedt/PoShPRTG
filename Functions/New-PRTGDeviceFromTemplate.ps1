function New-PRTGDeviceFromTemplate {
    ...
    Write-Log -LogText "Check if device already existis in the destination" -LogType Query -LogScope $Local:logscope -NoFileStatus
    ...
    if($Device -and ($Device.name -in $DestinationContent.name)) {
        Write-Log -LogText "$DeviceName ist unter ""$((Get-PRTGObject -ID $Destination -SensorTree $SensorTree -Verbose:$false).fullname)"" bereits vorhanden!" -LogType Warning -LogScope $Local:logscope -NoFileStatus -Warning
        ...
        if($result -eq 1) {
            Write-Log -LogText "Abbruch..." -LogType Warning -LogScope $Local:logscope -NoFileStatus -Warning
            return
        }
    }
    ...
    #Start cloning 
    # Get "OS"-Device 
    Write-Log -LogText "Start cloning devicetemplate to destination ID $Destination" -LogType Query -LogScope $Local:logscope -NoFileStatus
    $TemplateSystemDevice = Get-PRTGObject -ID $TemplateSystem -SensorTree $SensorTree -Verbose:$false -ErrorAction Stop
    if(-not $TemplateSystemDevice) { Write-Log -LogText "Template not found!" -LogType Error -LogScope $Local:logscope -NoFileStatus -Error; return }
    ...
    if($TemplateRole) {
        ...
        Write-Log -LogText "Query role specific sensor(s) from $($TemplateRole.count) role template(s)" -LogType Query -LogScope $Local:logscope -NoFileStatus
        ...
        Write-Log -LogText "Found $($TemplateSensorsToCopy.count) role specific sensor(s)" -LogType Info -LogScope $Local:logscope -NoFileStatus
        
        #Filtering out sensors excluded from deployment process
        if($TemplateSensorFilter) {
            Write-Log -LogText "Filtering out role specific sensor(s) from (Filter: $([string]::Join(", ",$TemplateSensorFilter)))" -LogType Query -LogScope $Local:logscope -NoFileStatus
            foreach($filter in $TemplateSensorFilter) { 
                $TemplateSensorsToCopy = $TemplateSensorsToCopy | Where-Object name -NotLike $filter
            }
            Write-Log -LogText "$($TemplateSensorsToCopy.count) role specific sensor(s) left to clone after filtering" -LogType Info -LogScope $Local:logscope -NoFileStatus
        }
        
        #Start deployment of role sensors
        if($TemplateSensorsToCopy) { 
            Write-Log -LogText "Start deployment of $($TemplateSensorsToCopy.count) role specific sensor(s)" -LogType Info -LogScope $Local:logscope -NoFileStatus
            $script:count = 0
            ...
            foreach($item in $TemplateSensorsToCopy) { 
                ...
                if ($pscmdlet.ShouldProcess($NewCustomerServer.name, "Deploy ""$($item.name)")) {
                    Write-Log -LogText "Deploy ""$($item.name)"" to ""$($NewCustomerServer.name)""" -LogType Set -LogScope $Local:logscope -NoFileStatus
                    ...
                    try {
                        ...
                        Write-Log -LogText "New sensor copied (objID:$($SensorCopy.objid) name:""$($SensorCopy.name))""" -LogType Info -LogScope $Local:logscope -NoFileStatus -DebugOutput
                        
                        #enable object
                        Write-Log -LogText "Activate / unpause new sensor" -LogType Set -LogScope $Local:logscope -NoFileStatus -DebugOutput
                        ...
                        Write-Log -LogText "Setting tags from role template to ""$($NewCustomerServer.name)""" -LogType Set -LogScope $Local:logscope -NoFileStatus -DebugOutput
                        ...
                        Write-Log -LogText "$($CopyObjectCollection.Count) sensors created until now." -LogType Info -LogScope $Local:logscope -NoFileStatus
                        Remove-Variable x -Force -ErrorAction Ignore -Verbose:$false -Debug:$false -WhatIf:$false
                    } catch {
                        #Write-Log -LogText $ErrorEvent.Message -LogType Error -LogScope $Local:logscope -NoFileStatus -Error
            ...
            Write-Log -LogText "No role specific sensors to deploy" -LogType Warning -LogScope $Local:logscope -NoFileStatus
        }
    } else { 
        Write-Log -LogText "No role template selected" -LogType Warning -LogScope $Local:logscope -NoFileStatus
    }

    if ($pscmdlet.ShouldProcess($NewCustomerServer.name, "Resume from pause status")) {
        Write-Log -LogText "Resume ""$($NewCustomerServer.name)"" from pause status" -LogType set -LogScope $Local:logscope -NoFileStatus
    ...
    if($SensorTree) {
        Write-Log -LogText "Refresh SensorTree" -LogType Query -LogScope $Local:logscope -NoFileStatus
